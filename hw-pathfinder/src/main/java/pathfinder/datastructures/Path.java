...
/**
 * This represents an immutable path between two cartesian coordinate Ps, particularly
 ...
 * @param <P> the type representing node(point)
 */
public class Path<P> implements Iterable<Path<P>.Segment> {

    // AF(this) =
    //      first P in the path => start
    //      each "step" along the path between Ps => elements of list path, where
    //          path.get(0) is the first step from the start P to an intermediate P, and
    //          path.get(path.size() - 1) is the final step from an intermediate P to the end
    //      total cost along the path => cost
    //      the destination P in this path, opposite the start P => getEnd()
    ...
    /**
     * The P at the beginning of this path.
     */
    private P start;

    /**
     * The ordered sequence of segments representing a path between Ps.
     */
    private List<Segment> path;

    /**
     * Creates a new, empty path containing a start P. Essentially this represents a path
     * from the start P to itself with a total cost of "0".
     *
     * @param start The starting P of the path.
     */
    public Path(P start) {
        this.start = start;
        this.cost = 0;
    ...
    /**
     * Appends a new single segment to the end of this path, originating at the current last P
     ...
     * segmentCost}.
     *
     * @param newEnd      The P being added at the end of the segment being appended to this path
     ...
     */
    public Path<P> extend(P newEnd, double segmentCost) {
        checkRep();
        //
        Path<P> extendedPath = new Path<P>(start);
    ...
    /**
     * @return The P at the beginning of this path.
     */
    public P getStart() {
        return start;
    }
    ...
    /**
     * @return The P at the end of this path, which may be the start P if this path
     * contains no segments (i.e. this path is from the start P to itself).
     */
    public P getEnd() {
        if(path.size() == 0) {
            return start;
     ...
     * P and ending at the end P. In the case that this path represents a path between
     * the start P and itself, this iterator contains no elements. This iterator does not
     ...
     * both paths are empty, they are only equal if their starting P is equal.
     *
    ...
    /**
     * Segment represents a single segment as part of a longer, more complex path between Ps.
     * Segments are immutable parts of a larger path that cannot be instantiated directly, and
     * are created as part of larger paths by calling Path#extend(P, double).
     */
    public class Segment {
         ...
         *
         * @param start The starting P of this segment.
         * @param end   The ending P of this segment.
         * @param cost  The cost of travelling this segment.
         * @throws NullPointerException     if either P is null.
         * @throws IllegalArgumentException if cost is infinite or NaN
         */
        private Segment(P start, P end, double cost) {
            if(start == null || end == null) {
                throw new NullPointerException("Segments cannot have null Ps.");
            }
        ...
        /**
         * @return The beginning P of this segment.
         */
        public P getStart() {
            // Note: Since Ps are immutable, this isn't rep exposure.
            return this.start;
        }

        /**
         * @return The ending P of this segment.
         */
        public P getEnd() {
            return this.end;
        }
