...
public final class RatPolyStack implements Iterable<RatPoly> {
    ...
    public RatPoly getNthFromTop(int index) {
        // {inv: the current recursion is the index - the number of the recursion}
        if (index != 0) { // if it is not the expected index, then pass into next recursion
            RatPoly temp = pop();
            RatPoly result =  getNthFromTop(index - 1);
            push(temp);
            return result;
        } else { // if it is the expected index, then return the peek
            return polys.peek();
        }
    }
