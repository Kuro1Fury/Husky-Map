...
public final class RatPoly {
    ...
    public RatTerm getTerm(int deg) {
        // {inv: expt of terms_0, ..., terms_i-1 is not equal to deg}
        for (int i = 0; i < terms.size(); i++) {
    ...
    public boolean isNaN() {
        // {inv: expt of terms_0, ..., terms_i-1 is not NoN}
        for (int i = 0; i < terms.size(); i++) {
    ...
    private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
        ...
        if (scalar.equals(RatNum.ZERO)) {
            ...
            // {inv: coeff of lst_0, ..., lst_i-1 = scalar * lst_pre_0, ..., scalar * lst_pre_i-1}
    ...
    private static void incremExpt(List<RatTerm> lst, int degree) {
        // Do not leave this method as-is. You must either use it somehow or remove it.
        if (degree != 0) {
            // {inv: degree of lst_0, ..., lst_i-1 = degree + lst_pre_0, ..., degree + lst_pre_i-1}
            for (int i = 0; i < lst.size(); i++) {
    ...
    private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
        ...
        int l = -1;
        int r = lst.size();
        // {inv: degree of lst_0, ..., lst_L <= newTerms.expt AND degree of lst_R, ..., lst_n > newTerms.expt}
        while (l + 1 != r) {
            int mid = (l + r) / 2;
            if (lst.get(mid).getExpt() > newTerm.getExpt()) {
                l = mid;
            } else {
                r = mid;
            }
        }
        if (r != lst.size() && lst.get(r).getExpt() == newTerm.getExpt()) {
            if (lst.get(r).getCoeff().add(newTerm.getCoeff()).equals(RatNum.ZERO)) {
                lst.remove(r);// if zero after adding, then remove that term
            } else {
                lst.set(r, lst.get(r).add(newTerm));
            }
    ...
    /**
     * Return an identical copy of List of RatTerm which is exactly the same with this.terms
     *
     * @return a List of RatTerm equal to this.terms; if this.isNaN(), returns some r such that r.isNaN()
     */
    private static List<RatTerm> getCopy(List<RatTerm> a) {
        List<RatTerm> temp = new ArrayList<RatTerm>();
        // {inv: temp = term_0, ..., term_i-1}
    ...
    public RatPoly negate() {
        ...
        List<RatTerm> temp = getCopy(terms);
    ...
    public RatPoly add(RatPoly p) {
        ...
        List<RatTerm> temp = getCopy(p.terms);
        // {inv: terms = terms_pre + p_0, ..., p_i-1}
        for (int i = 0; i < terms.size(); i++) {
    ...
    public RatPoly mul(RatPoly p) {
        ...
        // {inv: temp = terms * (p_0, ..., p_i-1)}
        for (int i = 0; i < p.terms.size(); i++) {
            List<RatTerm> copy = getCopy(this.terms);
            scaleCoeff(copy, p.terms.get(i).getCoeff()); // scale the copy based on the current coeff
            incremExpt(copy, p.terms.get(i).getExpt()); // increase degree based on the current expt
            temp = temp.add(new RatPoly(copy));
        }
        temp.checkRep();
        return temp;
    }
    ...
    public RatPoly div(RatPoly p) {
        ...
        RatPoly m = new RatPoly(); // result
        RatPoly n = new RatPoly(getCopy(this.terms)); // remainder
        int d = n.degree() - p.degree(); // difference between the degree of remainder and denominator
        // {inv: d + degree of q = degree of remainder AND n = p - q * m}
        while (d >= 0 && !n.terms.isEmpty()) {
            RatNum k = n.terms.get(0).getCoeff().div(p.terms.get(0).getCoeff()); // calculate proportion of coeff
            RatTerm t = new RatTerm(k, d); // calculate multiplier term
            RatPoly s = p.mul(new RatPoly(t)); // calculate the poly that need to be subtracted
            if (!t.isZero()) {
                m = m.add(new RatPoly(t)); // if not Zero, add multiplier to the result
            }
            n = n.sub(s); // subtract the remainder
            d = n.degree() - p.degree(); // update the difference of degree
        }
        m.checkRep();
        return m;
    }
    ...
    public double eval(double d) {
        ...
        double sum = 0;
        // {inv: sum = eval of terms_0, ... terms_i-1}
        for (int i = 0; i < terms.size(); i++) {
            sum += terms.get(i).eval(d);
        }
        return sum;
    }
